/**
 * VALIDATION.JS — FASE 2 (PARTE 1/2)
 * Avaliação clínica estruturada + Correlações CRS
 */
...
console.log("✅ validation.js carregado (PARTE 1/2)");

// ============================================
// CORRELAÇÕES CRS (Pearson)
// ============================================
...
function calculatePValue(r, n) {
  ...
  return Math.abs(r) > 0.7 ? 0.01 : Math.abs(r) > 0.5 ? 0.05 : 0.1;
}

// ============================================
// EXTRAIR DADOS CRS DA SESSÃO
// ============================================

function extractCRSData(session) {
  if (!session.audioBuffer || session.audioBuffer.length === 0) {
    return null;
  }

  const data = session.audioBuffer;
  return {
...
function calculateCorrelations(validation, crsData) {
  if (!crsData) return null;
  ...
  const crsMetrics = [
    ...
    { name: "Sub (< 250Hz)", data: crsData.sub },
    { name: "Low (250-500Hz)", data: crsData.low },
    { name: "Mid (500-2kHz)", data: crsData.mid },
    { name: "High (> 2kHz)", data: crsData.high }
  ];
  ...
  scales.forEach(scale => {
    ...
    crsMetrics.forEach(metric => {
      ...
      const p = calculatePValue(r, metric.data.length);
...
// ============================================
// GERAR RELATÓRIO DE CORRELAÇÃO
// ============================================

function generateCorrelationReport(validation, correlations) {
  if (!correlations) return null;

  const report = {
    sessionId: validation.sessionId,
    validationId: validation.id,
    medico: validation.medico,
    paciente: validation.paciente,
    dataValidacao: formatDate(validation.createdAt),
  ...
  Object.entries(correlations).forEach(([scaleName, metrics]) => {
    Object.entries(metrics).forEach(([metricName, corr]) => {
      ...
      report.summary.totalCorrelations++;
      if (corr.significant) {
        report.summary.significantCorrelations++;
      }
    });
  });

  // Top 5 correlações
  report.summary.topCorrelations = allCorrs
...
// ============================================
// SALVAR RELATÓRIO DE CORRELAÇÃO
// ============================================

function saveCorrelationReport(report) {
  const KEY_REPORTS = "elayon_correlation_reports";
  const reports = JSON.parse(localStorage.getItem(KEY_REPORTS) || "[]");
...
// ============================================
// EVENTO: SALVAR COM CORRELAÇÕES
// ============================================

const originalSalvar = btnSalvar.onclick;
...
btnSalvar.addEventListener("click", () => {
  ...
  // Calcular correlações
  const crsData = extractCRSData(currentSession);
  if (crsData) {
    const correlations = calculateCorrelations(validation, crsData);
    if (correlations) {
      const report = generateCorrelationReport(validation, correlations);
      saveCorrelationReport(report);
      showAlert("✅ Avaliação salva + Correlações calculadas!", "success");
    }
  } else {
    showAlert("✅ Avaliação salva (sem dados CRS para correlação)", "success");
  }
...
console.log("✅ validation.js carregado (PARTE 2/2)");
console.log("📊 Sistema de correlações CRS ativo");
console.log("🔬 Relatórios de validação habilitados");